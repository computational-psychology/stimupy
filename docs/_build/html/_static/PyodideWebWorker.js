...
async function loadApplication(cell_id, path) {
  ...
  const packages = ['pyodide-http', 'panel', 'param', 'matplotlib', 'stimupy'];
  if (path != null) {
  ...
  await self.pyodide.runPythonAsync("url='http://localhost:8000/_static/display_stimulus.py'; import pyodide.http; url_contents=pyodide.http.open_url(url).getvalue(); f=open('display_stimulus.py', 'w'); f.write(url_contents); f.close()")
