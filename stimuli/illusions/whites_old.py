...
###################################
def circular_white(shape, ppd, n_parts, target_locations, padding=(10,10,10,10,), high = 1., low = 0., target = .5, res = 400):

    height, width = stimuli.utils.degrees_to_pixels(shape, ppd)

    n_numbers = max(width, height)
    n_targets = len(target_locations)
    target_locations = np.array(target_locations)
    # Create a circle
    x = np.linspace(0, 2*np.pi, n_numbers)

    yy, xx = stimuli.utils.get_circle_indices(n_numbers, shape)
    # Change the background to gray and place the circle on the grid:
    grid = np.ones([height, width]) * target
    ...
    for i in range(n_parts-1):
        ...
        for j in range(n_numbers):
            r_tiny = r_chosen[j]
            xxx = r_tiny*xx
            xxx = xxx - xxx.max() / 2 + width/2
            yyy = r_tiny*yy
            yyy = yyy - yyy.max() / 2 + height/2
    ...
    padding = stimuli.utils.degrees_to_pixels(padding, ppd)
...
def RHS2007_WE_thick(ppd):
    phase_width = 2*ppd
    height = phase_width * 6
    n_cycles = 4
    padding_left = (1024-phase_width*n_cycles*2)//2
    padding_right = 1024 - padding_left - phase_width*n_cycles*2
    padding_top = (1024 - height)//2
    padding_bottom = 1024 - padding_top - height
    padding = (padding_top, padding_bottom, padding_left, padding_right)
    return default_white(n_cycles=n_cycles, grating_shape=(height, phase_width), target_index=2, target_spacing=3,
                                        target_height=phase_width*2, padding=padding, start="low")
def RHS2007_WE_thin_wide(ppd):
    phase_width = ppd
    ...
    padding_bottom = 1024 - padding_top - height
    padding = (padding_top, padding_bottom, padding_left, padding_right)
    return default_white(n_cycles=n_cycles, grating_shape=(height, phase_width), target_index=3, target_spacing=9,
                                    target_height=phase_width*2, padding=padding, start="high")

def RHS2007_WE_circular1(ppd):
    return circular_white((16,16), ppd, 8, (4,), padding=(8,8,8,8))

def RHS2007_WE_circular05(ppd):
    return circular_white(16, (10,), padding=(200, 200, 0, 0))

def RHS2007_WE_circular025(ppd):
